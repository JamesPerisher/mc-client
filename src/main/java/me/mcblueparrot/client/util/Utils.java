...
public class Utils {
	...
	public final static ExecutorService MAIN_EXECUTOR = Executors.newFixedThreadPool(Math.max(Runtime.getRuntime().availableProcessors(), 2));
	public static Comparator<String> STRING_WIDTH_COMPARATOR = Comparator.comparingInt(Utils::getStringWidth);
	...
	public static void glColour(Colour color) {
		GL11.glColor4ub((byte) color.getRed(), (byte) color.getGreen(), (byte) color.getBlue(), (byte) color.getAlpha());
	}

	public static void drawHorizontalLine(int startX, int endX, int y, int colour) {
		if(endX < startX) {
			int i = startX;
			startX = endX;
			endX = i;
	...
	public static void drawVerticalLine(int x, int startY, int endY, int colour) {
		if(endY < startY) {
			int i = startY;
			startY = endY;
			endY = i;
	...
	public static void drawOutline(Rectangle rectangle, Colour colour) {
		drawOutline(rectangle.getX(), rectangle.getY(), rectangle.getX() + rectangle.getWidth(), rectangle.getY() + rectangle.getHeight(),
				colour.getValue());
	}

	public static void drawOutline(int left, int top, int right, int bottom, int colour) {
	...
	public static void drawRectangle(Rectangle rectangle, Colour colour) {
	...
	public static int blendColor(int start, int end, float percent) {
		if(percent >= 1) {
			return end;
		}
	...
	public static URL sneakyParse(String url) throws MalformedURLException {
		return new URL(url);
	}
	 ...
	 */
	public static void drawCircle(float xx, float yy, int radius, int col) {
	...
	public static GuiChat getChatGui() {
